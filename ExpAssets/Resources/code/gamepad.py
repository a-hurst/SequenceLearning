...
class ControllerButton(object):
    ...
    @property
    def name(self):
        b = gc.SDL_GameControllerGetStringForButton(self._e.cbutton.button)
        return b.decode('utf-8')

    @property
    def timestamp(self):
        return self._e.cbutton.timestamp

    @property
    def state(self):
        return self._e.cbutton.state
...
def get_buttons(events, device=None, on_release=False):
    btype = SDL_CONTROLLERBUTTONUP if on_release else SDL_CONTROLLERBUTTONDOWN
    buttons = []
    for e in events:
        if e.type == btype:
